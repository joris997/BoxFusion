...
def get_camera_intrinsics()->o3d.camera.PinholeCameraIntrinsic:
...
class OnlineBoxAndPoint(Node):
    def __init__(self):
        ...
        # load the camera intrinsics from the calibration file
        self.camera_intrinsics = get_camera_intrinsics()

        #! Boxes stuff
        # create publisher as list of PolygonStamped
        self.box_visual_publisher = self.create_publisher(
            MarkerArray, 
            'online_3d_boxes', 
            10
        )
        # record boxes subscriber
        self.record_sub = self.create_subscription(
            Bool,
            '/open3d/record_boxes',
            self.record_boxes_callback,
            10
        )
        self.boxes = []
        ...
        is_depth_model = True
        self.model = make_cubify_transformer(dimension=backbone_embedding_dimension, depth_model=is_depth_model).eval()
        ...
        if self.cfg['device'] is not None:
            ...
            self.clip_model, self.preprocess = load_clip(self.cfg['clip_path'])
            ...
            extra_features = text_to_feature(self.objects, self.clip_model, device=self.cfg['device'])
    ...
    def save_boxes(self,save_path:str=None):
        ...
        for i, box in enumerate(self.boxes):
            ...
            if self.cfg['data']['use_o3d_pc']:
                xyzrgb = self.xyzrgb_o3d
            else:
                xyzrgb = self.xyzrgb_bf
            in_box_points = points_in_box(poly, xyzrgb=xyzrgb)
    ...
    def next_frame_callback(self, msg:Bool):
        ...
        if msg.data and self.cnt_pc < self.max_cnt_pc:
            self.cnt_pc += 1
            self.run_add_pointcloud()
    ...
    def run_add_pointcloud(self):
        for sample in self.dataset:
            ...
            self.xyzrgb_o3d = np.hstack((np.asarray(xyzrgb.points), np.asarray(xyzrgb.colors)))
            # save to .npy
            np.save(os.path.join(self.cfg['data']['output_dir'], f'xyzrgb_o3d.npy'), self.xyzrgb_o3d)

            self.publish_point_cloud(self.xyzrgb_o3d)
    ...
    def run_get_boxes(self,duration:float=10.0, gap:int=25, re_vis:bool=True):
        is_depth_model = "wide/depth" in self.augmentor.measurement_keys
        ...
        for sample in self.dataset:
            ...
            packaged = self.preprocessor.preprocess([packaged])
            ...
            if count % gap ==0 or count == len(self.dataset)-1:
                ...
                if all_pred_box is None and count<gap:
                    ...
                    class_results, box_features = text_prompt(boxes, self.tokenized_text, 
                                                              self.text_features, self.priority_features,
                                                              image, self.clip_model, self.preprocess) #[N_box]
        ...
        class_list = self.tokenized_text.tolist()
        ...
        if self.cfg['data']['output_dir'] is not None:# and self.cfg["eval"]:
            ...
            self.xyzrgb_bf = xyzrgb.cpu().numpy()
            # torch.save(xyzrgb.cpu(), os.path.join(self.cfg['data']['output_dir'], 'xyzrgb.pt'))
            np.save(os.path.join(self.cfg['data']['output_dir'], 'xyzrgb_bf.npy'), self.xyzrgb_bf)
...
def main(args=None):
    ...
    try:
        ...
        node.get_logger().info("Shutting down online Box and Point node")
        node.destroy_node()
        rclpy.shutdown()
